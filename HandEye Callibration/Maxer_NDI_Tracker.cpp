...
unsigned int NDITracker::AddTool(char* fileName)
{
...
int NDITracker::ConvertBinaryFileToAsciiEncodedHex(char *asciiEncodedHexStr, char * fileName)
{
...
int NDITracker::GetTrackingData ( int portNum, double transform[8])
{
	      ...
	      return  ndiGetTXTransform(device, portNum,  transform);
}
